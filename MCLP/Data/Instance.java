...
public class Instance
{
	...
	public Instance(InputParameters leitor)
	{
		...
		this.numFacilidades = leitor.getP();
		this.raio = leitor.getR();
		...
		try
		{
			...
			while(!str[0].trim().equals("EOF"))
			{
				switch(str[0].trim()) {
				case "DIMENSION":
					size = Integer.valueOf(str[1].trim());
					break;
				case "EDGE_WEIGHT_TYPE":
				case "EDGE_WEIGHT_FORMAT":
					setTipoCoord(str[1].trim());
					break;
...
//	}
	...
	private float distanciaCoord(double Lat1, double Long1, double Lat2, double Long2)
	{
		...
		return (float) Math
		.acos(Math.cos(firstLatToRad) * Math.cos(secondLatToRad) * Math.cos(deltaLongitudeInRad) + Math.sin(firstLatToRad) * Math.sin(secondLatToRad))
		* 6378100;
	}
